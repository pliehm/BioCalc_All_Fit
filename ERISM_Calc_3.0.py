########################################################################################
### Script to calculate the cavity thicknes of an ERISM micro-cavity for every pixel ###
########################################################################################
...
# the values have to correspond to the folders in the folder list, i.e. "data"
init_guess = [7690] 
...
enhance_resolution = 1


# chose elastomer thickness range , the smaller the range the faster the program. If you are not sure, just take d_min = 3000, d_max = 15000
...
for data_folder in data:
    ...
    # remove results folder from list in case it is already created
    counter = 0
    for item in folder_list:
        if item == "One Wavelength Images":
            folder_list.pop(counter)
        if item == "Transmission Images":
            folder_list.pop(counter)
        if item == data_folder + "_Thickness_maps":
            folder_list.pop(counter)
        else:
            counter += 1

    # make new folder for results if it does not exist
    if not os.path.exists(data_folder + "/" + data_folder + "_Thickness_maps"):
        os.mkdir(data_folder + "/" + data_folder + "_Thickness_maps")
...
#######################
        ...
        # generate a useful filename for txt file
        file_name = data_folder + '_' + folder + time.strftime("_%Y%m%d_%H%M%S")+'.txt'

        # generate a useful filename for tif file
        file_name = data_folder + '_' + folder + time.strftime("_%Y%m%d_%H%M%S")+'.tif'
        ...
        imsave(data_folder + "/" + data_folder + "_Thickness_maps" + "/" + file_name,result)


        # update the guess for each video frame in case the value in the upper left corner changes due to a cell
        if one_minimum_fit == True:
            print "result[0][0]: ", result[0][0]
            if np.isnan(result[0][0]) != True:
                init_guess[folder_counter] = result[0][0]
                print "new initial guess is: ", init_guess[folder_counter]
