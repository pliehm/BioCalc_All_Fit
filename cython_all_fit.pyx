...
cdef list peakdetect(np.ndarray[DTYPE_t, ndim=1] y_axis, list x_axis = None, unsigned short lookahead_min=5, unsigned short lookahead_max=3, unsigned short delta = 0,unsigned short enhance_resolution=10):    
...
#
    ...
    if enhance_resolution != 1:
        ...
        window_len = 5*enhance_resolution # this needs to be checked, maybe it can be smaller or has to be larger
    ...
    if use_thickness_limits:
        ...
        for row in range(len(all_images[0])):
            ...
            for column in xrange(Image_width):
                ...
                minima_exp = np.array(peakdetect(intensity, waves, lookahead_min*enhance_resolution,lookahead_max*enhance_resolution, delta,enhance_resolution),dtype=np.float)
                ...
                if (last_thickness != 0) and (last_index > 0):
                    ...
                    if current_thickness == 0: 
                        for new_delta in range(1,5):
                            ...
                            minima_exp = np.array(peakdetect(intensity, waves, lookahead_min*enhance_resolution,lookahead_max*enhance_resolution, delta + new_delta*delta_vary,enhance_resolution),dtype=np.float)
                            current_thickness, current_index, sim_min_waves = (Fit_limit(thickness,array_thickness_len_pos, array_length_block, minima_exp,tolerance,all_minima, last_index, thickness_list,thickness_limit))
